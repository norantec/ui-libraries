  ...
  return await new Promise((resolve) => {
    ...
    const handleGetItemValueResponse = (
      ...
      if (resultMap.size === registeredFieldNames.length) {
        ...
        resolve(
          Object.fromEntries(Array.from(resultMap.entries()).filter(([, value]) => typeof value !== 'undefined')),
        );
      }
    };
  ...
  public async validate(inputNames?: string[]) {
    ...
    return await new Promise<FormValidateResult>((resolve) => {
      ...
      const handleResult = (id: string, eventMessage: EventMessage<EventName.VALIDATE_ITEM_RESULT>) => {
        if (id !== this.id || eventMessage?.data?.[0] !== requestId || !names.includes(eventMessage?.data?.[1])) {
          return;
        }
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      if (value === null) {
        ...
        eventEmitter.emit(EventName.SET_ITEMS_VALUE, idRef.current, new EventMessage(EventName.SET_ITEMS_VALUE, value));
      }
    }, 0);
  }, [value, idRef.current]);
...
Form.Item = function (inputProps: FormItemProps) {
  ...
  usePreviousValueEffect(
    () => {
      setTimeout(() => {
        eventEmitter.emit(EventName.ITEMS_VALUE_CHANGE, id, new EventMessage(EventName.ITEMS_VALUE_CHANGE, null));
      }, 0);
    },
  ...
  useEffect(() => {
    const handleSetItemValue = (currentId: string, eventMessage: EventMessage<EventName.SET_ITEMS_VALUE>) => {
      if (id !== currentId || StringUtil.isFalsyString(name) || !Object.keys(eventMessage.data ?? {}).includes(name)) {
        return;
      }
  ...
  useEffect(() => {
    const handleValidateItems = (currentId: string, eventMessage: EventMessage<EventName.VALIDATE_ITEMS>) => {
      ...
      const submitValidators = normalizedValidators.filter((validator) => validator?.validateOnValidation !== false);
  ...
  useEffect(() => {
    ...
    Promise.all(
      changeValidators.map((validator) => Promise.resolve(validator.validate(valueRef.current, formValueRef.current))),
    )
      .then((resultList) => {
        return resultList.filter((result) => !StringUtil.isFalsyString(result));
      })
        ...
        {children &&
          React.cloneElement(children, {
            ...
            onChange: (value: any, ...others: any[]) => {
              const outgoingValue = (() => {
                let result: any;
                if ((value as any)?._reactName === 'onChange' || (value as React.BaseSyntheticEvent)?.target) {
